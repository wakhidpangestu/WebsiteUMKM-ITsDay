<h1 align="center">🥟 Tong Hee Love</h1>
<p align="center">
  <strong>UMKM Siomay Asli Bandung | Sejak 1998</strong><br/>
  Website modern untuk branding, promosi, dan pemesanan siomay berkualitas khas Bandung.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Next.js-14.2.23-blue?logo=nextdotjs" />
  <img src="https://img.shields.io/badge/TailwindCSS-Styled-38B2AC?logo=tailwindcss" />
  <img src="https://img.shields.io/badge/shadcn/UI-Modern-black?logo=vercel" />
  <img src="https://img.shields.io/badge/Status-Publish-green" />
</p>

---

## 🚀 Preview

![Promo Tong Hee Love](./public/poster-somay.png)

> 🌐 **Live Website:** _→ [tongheelove.vercel.app](https://tongheelove.verce.app/)_

---

## 📌 Tentang Tong Hee Love

**Tong Hee Love** adalah usaha siomay rumahan yang telah berdiri sejak **1998**, menghadirkan rasa otentik siomay Bandung. Menu kami lengkap, porsi hemat, dan pastinya bikin nagih!

### 📋 Menu:
- 🥟 Siomay
- 🥬 Pare & Kol
- 🥔 Kentang
- 🍢 Tahu
- 🐟 Batagor

💰 **Harga:**
- Rp12.000 per porsi
- Rp2.000 per biji

📍 **Alamat:** Halte Pondok Sukmajaya Permai  
📞 **Kontak:** [0812-8282-9139](tel:081282829139)  
🕐 **Jam Buka:** 10.00 – 19.00  
💸 **Pembayaran:** Tunai, QRIS, Transfer  
🚚 **Delivery:** Bisa antar, tergantung mood (alias free delivery kalau hoki 😄)  
📦 **Minimum Order:** Tidak ada! (pesanan banyak, DP dulu ya)

---

## 🌟 Fitur Website

- 🎯 Call-to-action WhatsApp langsung pesan
- 🌗 Dark / Light mode
- 📷 Gambar produk menarik & responsif
- 🎁 Highlight promo & testimoni
- 📍 Google Maps embed untuk lokasi
- 📱 Mobile-first design

---

## 🛠️ Tech Stack

<table>
  <tr>
    <td align="center" valign="middle">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="40" /><br/>
      <strong>React</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="40" /><br/>
      <strong>Next.js</strong>
    </td>
    <td align="center" valign="middle">
        <img src="https://assets.vercel.com/image/upload/front/favicon/vercel/180x180.png" width="40" /><br/>
        <strong>Vercel</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://upload.wikimedia.org/wikipedia/commons/d/d5/Tailwind_CSS_Logo.svg" width="40" /><br/>
      <strong>Tailwind CSS</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://raw.githubusercontent.com/shadcn/ui/main/apps/www/public/favicon.ico" width="40" /><br/>
      <strong>shadcn/ui</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://avatars.githubusercontent.com/u/75042453?s=200&v=4" width="40" /><br/>
      <strong>Radix UI</strong>
    </td>
  </tr>
  <tr>
    <td align="center" valign="middle">
      <img src="https://api.iconify.design/simple-icons/lucide.svg" width="40" /><br/>
      <strong>Lucide</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="40" /><br/>
      <strong>JavaScript</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://raw.githubusercontent.com/simple-icons/simple-icons/develop/icons/lodash.svg" width="40" /><br/>
      <strong>Lodash</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://api.iconify.design/logos/google-maps.svg" width="40" /><br/>
      <strong>Google Maps</strong>
    </td>
    <td align="center" valign="middle">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/webpack/webpack-original.svg" width="40" /><br/>
      <strong>Webpack</strong>
    </td>
  </tr>
</table>

---

## ⚙️ Instalasi Lokal

```bash
git clone https://github.com/username/tong-hee-love.git
cd tong-hee-love
npm install
npm run dev
```
